...
**這個問題現在會自動解決，不需要手動操作**：App 一啟動就會在背景把全部 3,060 個字的
發音（約 30 MB，用實際測得的平均每個字約 9.8 KB 換算）自動下載並存進跟上面同一個
Cache Storage，「學習進度」頁面的「🔊 離線發音快取」只是顯示目前下載進度的狀態列，沒有
按鈕、沒有選項——下載完之後每一個字都已經「快取過」了，不管在哪裡、網路好不好、甚至
完全離線，發音都會直接從裝置本身秒開，不會再有任何網路延遲或音質下降的問題。已經下載過
的字會自動跳過（不會重複耗用流量），如果啟動當下沒有網路，恢復連線後也會自動繼續下載，
不需要重新整理或做任何操作。
