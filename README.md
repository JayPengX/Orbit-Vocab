...
使用——安裝時**不會**把全部 3,060 個單字的音檔都預先下載下來（那樣單次安裝就要抓好幾十
MB），預設只有實際播放過的單字音檔才會被快取，之後不管有沒有網路都能重播；想一次把全部
音檔準備好、讓發音完全不受網路影響的話，可以在「學習進度」頁面用「🔊 離線發音快取」
區塊手動下載，見下方〈單字發音〉。
...
**為什麼網路不好時發音會變慢／變差**：每個字的發音第一次播放時都得先跟伺服器要那個字的
mp3 檔案，拿到之後才能播；拿到過一次之後，瀏覽器就會把它存進本機的 Cache Storage（見
上方〈加到主畫面與離線使用〉），下一次不管網路好不好都是直接從裝置本身讀取，不會再受
網路影響。所以真正會被網路拖慢的，只有「這台裝置從來沒播放過的字」——網路不好時，抓
這種字的第一次發音本來就比較久，逾時（3 秒）之後還會直接退回音質較差的瀏覽器語音合成，
這就是「網路不好時聲音變慢又變差」的完整原因。

**「學習進度」頁面的「🔊 離線發音快取」**就是讓這個問題徹底不會發生的辦法：按一下
「下載全部發音」就會把全部 3,060 個字的發音（約 30 MB，用實際測得的平均每個字約 9.8 KB
換算）都下載並存進跟上面同一個 Cache Storage，下載完之後每一個字都已經「播放過」了，
不管在哪裡、網路好不好、甚至完全離線，發音都會直接從裝置本身秒開，不會再有任何網路延遲
或音質下降的問題。已經下載過的字會自動跳過（不會重複耗用流量），可以隨時按「取消下載」
中斷，之後再回來按一次「下載全部發音」會從上次停下的地方繼續，不會從頭來過。
