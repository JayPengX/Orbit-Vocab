...
〈跨裝置同步〉已經設定好 `PROXY_URL`，這兩個功能就會一起自動可用（`PROXY_URL` 留空時，
下面兩個入口都會直接消失，不會出現按了會失敗的死按鈕）：

- **🪄 AI 記憶法**（「學習進度」頁面〈單字明細〉表格，只出現在有實際答錯紀錄的單字列）：
  依這個單字「你自己實際打錯過的拼法」（`recentWrongAnswers`），請 AI 針對那個錯誤模式
  產生一個專屬的記憶提示——跟同一張表格旁邊、答錯當下就會顯示的靜態提示（`data/
  ...
  （例如請求過於頻繁）。
- **📖 AI 記憶故事**（「複習」頁面〈卡片複習模式〉面板下方，跟卡片複習共用同一個分類
  選擇——❌ 答錯待複習／📖 學習中）：從目前選的分類裡，依跟卡片複習模式相同的風險預測
  模型（`Logic.rankCandidates`）挑最多 6 個單字，請 AI 編一則同時用到這些單字的短故事，
  幫助把它們串在一起記憶。分類目前的單字數不足 2 個（產生故事的最低需求）時，按鈕會停用
  並顯示原因。

兩者都是純文字生成，不會碰到 `/vocab-sync` 的 Firestore 資料，也不需要知道你有沒有設定
跨裝置同步；沒有同步、純本機使用的裝置一樣能用。實作見 `vocab-ai.js`（只負責呼叫 Worker，
沒有自己的畫面）與 `app.js` 裡呼叫它的兩處按鈕邏輯。
...
- `vocab-ai.js`：個人化記憶法／AI 記憶故事——透過同一支 Cloudflare Worker 呼叫即時 Gemini
